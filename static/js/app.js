function buildMetadata(sample) {
  ...
  var Side_Data = d3.select("#sample-metadata");
  Side_Data.html("");
 
  var data = Object.entries(response);

  data.forEach(function(item) {
    Side_Data.append("div").text(item);});
 })}
...
function init() {
  // Obtain a reference to the dropdown select element
...
function optionChanged(newSample) {
  // Show new data each time a new sample is selected
